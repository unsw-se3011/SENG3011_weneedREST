...
For the API, issues with the endpoints were becoming more complicated. Due to a lack of proficiency with Flask-Restplus API's, we were not able to make the API endpoints in accordance to the specification. After a thorough read through of the documentation, use of namespaces, Classes (manager/mediator design pattern), marshal module and other improvements we were able to completely refractor the code and reduce code significantly.
